...
//	enctype 속성이 multipart/form-data로 설정된 폼에서 넘어오는 데이터를 받으려면 request로 받을 수 없고
//	multipart를 지원하는 객체로 받아야 한다.
	try {
		...
		//	실제로 디스크에 업로드 된 파일 이름을 얻어온다.	
		//	getFile(): 파일이 실제로 업로드 된 경로 이름과 파일 이름을 얻어온다. 
		...
		out.println("업로드 용량 초과, 5MB까지만 업로드 가능");
	}
