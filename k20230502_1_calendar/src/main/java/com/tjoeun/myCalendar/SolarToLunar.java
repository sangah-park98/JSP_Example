	...
	public static ArrayList<LunarDate> solaToLunar(int year, int month) {
		...
		for (int i=1; i<=12; i++) {
			...
			try {
				...
				// org.jsoup.select 패키지의 Elements 클래스 객체에 Document 클래스 객체로 읽어드린 내용을
				...
				// Element 클래스 객체를 이용해서 Elements 클래스 객체에 저장된 데이터를 한 건씩 처리한다.
		...
		for (int i=0; i<lunarList.size(); i++) {
			...
			// 음력 공휴일 => 음력 1월 1일(설날), 음력 4월 8일(석가탄신일), 음력 8월 15일(추석) => 윤달일
			// 경우 공휴일이 아니다.
			...
			// 주말(토, 일)이나 다른 공휴일과 겹치면 그 다음 첫 번째 비공휴일을 대체 공휴일로 한다.
			...
			// 석가탄신일 대체 공휴일 => 석가탄신일 주말과 겹쳤을 때
			if (lunarList.get(i).getMonthLunar() == 4 && lunarList.get(i).getDayLunar() == 8
					 && !lunarList.get(i).isLunarFlag()) {
			...
			if (lunarList.get(i).getMonth() == 5 && lunarList.get(i).getDay() == 5
				...
				if (holiday == 6) {
					...
					// 어린이날과 석가탄신일이 금요일과 토요일을 제외한 나머지 요일에 겹쳤을 때
					lunarList.get(i + 1).setHoliday("<br><span>대체공휴일</span>");
				}
			}
			...
			// 크리스마스 대체 공휴일 => 크리스마스가 주말과 겹쳤을 때
			if (lunarList.get(i).getMonth() == 12 && lunarList.get(i).getDay() == 26) {
